 ...
 * 타입스크립트의 가장 기본적인 타입은 자바스크립트의 7가지 원시타입과 동일합니다.
 *
 * 타입스크립트의 타입 시스템은 다음과 같이 작동합니다.
 ...
 *
 * 구문 오류: 타입스크립트가 코드로 이해할 수 없는 잘못된 구문을 감지할 때 발생합니다.
 * 타입 오류: 타입스크립트의 타입 검사기가 프로그램의 타입에서 오류를 감지했을 때 발생합니다.
 ...
 * 타입스크립트 변수에 동일한 타입의 다른 값이 할당될 때는 아무런 문제가 없습니다.
 * 타입스크립트에서 함수 호출이나 변수에 값을 제공할 수 있는지 여부를 확인하는 것을 '할당 가능성'이라고 합니다.
 * 즉, 전달된 값이 예상된 타입으로 할당 가능한지 여부를 확인합니다.
 */

/** 타입 애너테이션
 * 때로는 변수에 타입스크립트가 읽어야 할 초깃값이 없는 경우도 있답니다. 타입스크립트는 나중에 사용할 변수의 초기 타입을 파악하려고 시도하지 않습니다.
...
/** 타입스크립트는 number 타입으로 진화한 변수가 toUpperCase() 메서드를 호출하는 것을 포착했습니다.
 * 그러나 변수가 string 타입에서 number 타입으로 진화된 것이 처음부터 의도된 것인지에 대한 여부는 더 일찍 알 수 없습니다.
 * 일반적으로 any 타입을 사용해 any 타입으로 진화하는 것을 허용하게 되면 타입스크립트의 타입 검사 목적을 부분적으로 쓸모없게 만듭니다.
 * 타입스크립트는 값이 어떤 타입인지 알고 있을 때, 가장 잘 동작합니다. any 타입을 가진 값에는 타입스크립트의 타입 검사 기능을 잘 적용할 수 없습니다.
 * 검사를 위해 알려진 타입이 없기 때문이죠.
 *
 * 타입스크립트는 초깃값을 할당하지 않고도 변수의 타입을 선언할 수 있는 구문인 '타입 애너테이션'을 제공합니다. 타입 애너테이션은 변수 이름 뒤에 배치되며
...
/** 불필요한 타입 애너테이션
 * 타입 애너테이션은 타입스크립트가 자체적으로 수집할 수 없는 정보를 타입스크립트에 제공할 수 있답니다.
 * 타입을 즉시 유추할 수 있는 변수에도 타입 애너테이션을 사용할 수 있습니다. 하지만 타입스크립트가 아직 알지 못하는 것을 알려주지 못합니다.
 *
